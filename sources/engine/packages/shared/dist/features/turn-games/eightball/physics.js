...
import { distance, dot, sub, unit, } from "./geometry.js";
import { BALL_R, CUE_ID, POCKETS, POCKET_IDS, TABLE_HEIGHT, TABLE_WIDTH } from "./types.js";
...
const RAIL_ORDER = ["top", "bottom", "left", "right"];
 ...
 */
export function assertFiniteSimResult(result) {
    const check = (n, label) => {
        if (!Number.isFinite(n))
            throw new Error(`physics: non-finite ${label}: ${n}`);
    };
    ...
    for (const p of result.events.potted)
        check(p.t, `potted[${p.ballId}].t`);
...
export function simulateShot(input) {
    ...
    for (const b of input.balls)
        active.set(b.id, { id: b.id, x: b.x, y: b.y, vx: b.vx, vy: b.vy });
    ...
    const finalFrameBalls = finalBallsArr.map((b) => ({
        id: b.id,
        x: round2(b.x),
        y: round2(b.y),
    }));
    ...
    for (const p of ctx.pendingPotForFrame)
        finalFrameBalls.push({ id: p.id, x: round2(p.x), y: round2(p.y) });
    finalFrameBalls.sort((a, b) => a.id - b.id);
    const finalFrame = { t: round2(duration), balls: finalFrameBalls };
    ...
    const finalBalls = finalBallsArr.map((b) => ({ id: b.id, x: b.x, y: b.y, vx: b.vx, vy: b.vy }));
    return {
        finalBalls,
        events: {
            potted: ctx.potted,
...
function runOuterStep(active, dt, ctx) {
    let remaining = dt;
    let iterations = 0;
...
function anyMoving(active) {
    for (const ball of active.values())
        if (ball.vx !== 0 || ball.vy !== 0)
            return true;
    return false;
}
function applyFriction(active) {
    for (const ball of active.values()) {
...
// ── event search ──────────────────────────────────────────────────────────────
function findEarliestEvent(active) {
    const ids = [...active.keys()].sort((x, y) => x - y);
    let best = null;
    const consider = (candidate) => {
        if (!candidate)
            return;
        if (!best || candidate.t < best.t - MIN_EVENT_T)
            best = candidate;
    };
    ...
    for (const id of ids) {
        ...
        for (const rail of RAIL_ORDER)
            consider(railCrossingTime(ball, rail));
    }
    for (const id of ids) {
        const ball = active.get(id);
        for (const pid of POCKET_IDS)
            consider(pocketCaptureTime(ball, pid));
    }
    return best;
}
...
function ballCollisionTime(a, b) {
    ...
    if (cCoef <= 0) {
        ...
        if (bCoef < -MIN_EVENT_T)
            return { kind: "ball", t: MIN_EVENT_T, a: a.id, b: b.id };
        return null;
    }
...
function railCrossingTime(ball, rail) {
    let t;
    let horizontal;
    switch (rail) {
        case "top":
            if (ball.vy >= -MIN_EVENT_T)
                return null;
...
function willReachPocket(ball, pocketId) {
...
function pocketCaptureTime(ball, pocketId) {
...
// ── event resolution ──────────────────────────────────────────────────────────
function resolveEvent(ev, active, ctx) {
    if (ev.kind === "ball")
        resolveBallCollision(ev.a, ev.b, active, ctx);
    else if (ev.kind === "rail")
        resolveRailReflection(ev.id, ev.rail, active, ctx);
    else
        capturePot(ev.id, ev.pocketId, ctx.simTime, active, ctx);
}
...
function resolveBallCollision(idA, idB, active, ctx) {
    ...
    if (idA === CUE_ID || idB === CUE_ID) {
        ...
        if (ctx.firstContactBallId === null)
            ctx.firstContactBallId = other;
    }
}
...
function resolveRailReflection(id, rail, active, ctx) {
    const ball = active.get(id);
    switch (rail) {
        case "top":
...
function capturePot(ballId, pocketId, t, active, ctx) {
    const ball = active.get(ballId);
    if (!ball)
        return; // already removed (defensive; shouldn't occur)
 ...
 */
function forceCaptureOutOfBounds(active, ctx) {
    for (const ball of [...active.values()]) {
 ...
 * in the rail.
 */
function captureStoppedJawBalls(active, ctx) {
...
function maybeEmitFrame(active, ctx, stepCount) {
    if (stepCount % FRAME_STEP_INTERVAL !== 0)
        return;
