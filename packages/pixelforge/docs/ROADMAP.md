...
weather override as a consumer that is read-complete and writer-less; **W10** opens for procedural
world-history generation. Open questions: **3** absorbs `feature.flavor`, **8** is answered in part
...
| **weather** | `weather:<word>[:untilDay]`, with an optional `intensity` on rain and snow | **nothing — the read side is complete.** 0.14 ships the fold, the day-range predicate, the reconciler and the town's answer; the only writer is a browser console |
...
| **one tuning row** | `fishBite:<word>:<multiplier>` over the bite-rate table | nothing new; the table is five-valued and closed already |
...
**And the proof this item has been missing now exists in the package.** 0.14's weather override is a complete package-side consumer of exactly the channel P7 wants: a validated closed vocabulary, folded defensively, landing in chat metadata, re-read on every props delivery, with a re-place already wired — and **no writer**, because the host dispatches capability events on engine-defined type strings only. It is the smallest possible version of this item's second direction, built and dark, and it makes the ask concrete: the channel is the only missing piece, and it is missing for weather exactly as it is missing for money.
