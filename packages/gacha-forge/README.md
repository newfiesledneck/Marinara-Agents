...
**1.8.2 is the kits, and four bugs players reported.** Measured on three real worlds, 22 of 24 passives were ATK up, DEF down or a shield, and every character carried one: in the fight a buff was ATK up and nothing else, a debuff DEF down and nothing else, and the prompts that write units never said what the other effects do. A buff and a debuff now name their stat -- ATK, DEF, SPD (it moves the turn order) or crit chance up; DEF, ATK or SPD down -- through the fight, the unit sheet, the status chips and the power score, and a kit written before this fights byte for byte as it did. Every prompt that writes a unit or an enemy carries what each effect does as an Ultimate and as a passive, what each target reaches and when a weapon's skill fires, and a new character is told what the cast already has. Fixed: an outfit rotation with a theme from the Theme library was painted, lost at the save and minted again on every visit (a paid loop); a scene lost its line when the browser reloaded the tab; an unreadable connection list opened as an empty dropdown with no reason. A shared theme set is weighed before it is read. It registers 113 routes.

**1.8.5 is world creation, and the models that think.** Creating a world ended on the opening story -- chapter one's plan, its fights and its first scene: more than half of the wait and the heaviest calls of the whole creation -- and when that forge failed the only ways out were Retry and deleting the world. "Forge this world" now asks *are you sure?* and then *now or later*: later, the world opens on the Home as soon as its cast stands, with Summon unlocked and its starting gift to claim, nothing forges by itself, and Modes > Story asks before writing chapter one, whose first scene is still free; a failed opening forge grows a third way out, "Skip the story for now", which keeps the world. The wizard is saved: a draft kept on the server without asking and mounted back the next time world creation opens (held while the world it became is being created, back if that creation is cancelled, spent when it ends well), and up to 30 named presets -- saved from the wizard or from a world that already exists -- that carry every source and, when loaded, drop and name whatever the Engine no longer has. A player's GLM 5.3 ledger (149 calls) measured the other half: 82% of what came back was reasoning, 12 of 15 scenes and 3 of 3 chapter plans were cut at exactly their cap with no answer, and every doubled retry landed under 9.6k. The room to think goes from a fixed 6,000 to 16,000 and is then learned per connection from the ledger (half as much again for a call that finished thinking, double for one cut off, never over 64,000), it stays inside the connection's declared context window, the first budget cut of a generation spends no attempt, and the ledger records each call's duration and cap. Fixed: cards, lorebooks, connections, image options and personas were read once per screen and went stale while the player worked in Marinara; at minting, World lorebook keys were matched against the premise alone, so a card's own name never woke the entry about them. It registers 121 routes.
