...
import { canGenerateNoodleActivityForAccountKind } from "./slurp-prompt.js";
import {
...
export async function persistGeneratedNoodleActivity(input: {
  ...
  const existingInteractionById = new Map(
    (await input.noodle.listInteractions([...allowedPostIds])).map((interaction) => [interaction.id, interaction]),
  );
